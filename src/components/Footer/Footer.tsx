...
function PageLink({ label, page }: { label: string; page: { href: string; title: string } }) {
  return (
    <>
      <Button as={Link} href={page.href} aria-label={`${label}: ${page.title}`} variant="secondary">
        {label}
      </Button>
      <Link
        href={page.href}
    ...
    <div className="flex">
      {previousPage && (
        <div className="flex flex-col items-start gap-3">
          <PageLink label="Previous" page={previousPage} />
        </div>
      )}
      {nextPage && (
